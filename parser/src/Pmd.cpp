...
bool pmd::PmdHeader::parse(std::ifstream& stream)
{
	char buffer[256];
...
bool pmd::PmdHeader::parseExtension(std::ifstream& stream)
{
	char buffer[256];
	stream.read(buffer, 20);
...
bool pmd::PmdVertex::parse(std::ifstream& stream)
{
...
bool pmd::PmdMaterial::parse(std::ifstream& stream)
{
	char buffer[20];
...
void pmd::PmdBone::parse(std::istream& stream)
{
	char buffer[20];
	stream.read(buffer, 20);
	name = std::string(buffer);
...
void pmd::PmdBone::parseExpantion(std::istream& stream)
{
	char buffer[20];
	stream.read(buffer, 20);
	name_english = std::string(buffer);
...
void pmd::PmdIk::parse(std::istream& stream)
{
...
void pmd::PmdFaceVertex::parse(std::istream& stream)
{
	stream.read((char *)&vertex_index, sizeof(int));
	stream.read((char *)position, sizeof(float) * 3);
}

void pmd::PmdFace::parse(std::istream& stream)
{
	char buffer[20];
	stream.read(buffer, 20);
	name = std::string(buffer);
	...
	{
		vertices[i].parse(stream);
	}
}

void pmd::PmdFace::parseExpantion(std::istream& stream)
{
	char buffer[20];
	stream.read(buffer, 20);
	name_english = std::string(buffer);
...
void pmd::PmdBoneDispName::parse(std::istream& stream)
{
	char buffer[50];
...
void pmd::PmdBoneDispName::parseExpantion(std::istream& stream)
{
	char buffer[50];
	stream.read(buffer, 50);
...
void pmd::PmdBoneDisp::parse(std::istream& stream)
{
	stream.read((char*)&bone_index, sizeof(uint16_t));
	stream.read((char*)&bone_disp_index, sizeof(uint8_t));
}

void pmd::PmdRigidBody::parse(std::istream& stream)
{
	char buffer[20];
...
void pmd::PmdConstraint::parse(std::istream& stream)
{
	char buffer[20];
	stream.read(buffer, 20);
	name = std::string(buffer);
...
std::unique_ptr<PmdModel> pmd::PmdModel::LoadFromStream(std::ifstream& stream)
{
	...
	{
		result->vertices[i].parse(stream);
	}
	...
	{
		result->materials[i].parse(stream);
	}
	...
	{
		result->bone_disp_name[i].parse(stream);
	}
	...
	{
		result->bone_disp[i].parse(stream);
	}
	...
	if (english)
	{
		result->header.parseExtension(stream);
		for (uint32_t i = 0; i < bone_num; i++)
		{
			result->bones[i].parseExpantion(stream);
		}
		for (uint32_t i = 0; i < face_num; i++)
		{
			...
			result->faces[i].parseExpantion(stream);
		}
		for (uint32_t i = 0; i < result->bone_disp_name.size(); i++)
		{
			result->bone_disp_name[i].parseExpantion(stream);
		}
	}
	...
	if (stream.peek() == std::ios::traits_type::eof())
	{
		...
		{
			result->rigid_bodies[i].parse(stream);
		}
		...
		{
			result->constraints[i].parse(stream);
		}
	}
